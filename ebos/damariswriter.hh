    ...
    void writeOutput(data::Solution& localCellData , bool isSubStep)
    {
        ...
        if (! isSubStep) {
            ...
            for ( auto damVar : localCellData ) {
                ...
                damaris_err_ = damaris_set_position(name.c_str(), temp_int64_t);
                if (damaris_err_ != DAMARIS_OK && rank_ == 0) {
                    OpmLog::error(fmt::format("ERORR: damariswriter::writeOutput()       : ( rank:{}) damaris_set_position({}, ...), Damaris Error: {}  ",  rank_, name, damaris_error_string(damaris_err_) ));
                }
            }
            ...
            for ( auto damVar : localCellData ) {
              ...
              OpmLog::debug(fmt::format("Name of Damaris Variable       : ( rank:{})  name: {}  Size : {} ",  rank_, name, dataCol.data.size()));
              
              if (dataCol.data.data() != nullptr) {
                    damaris_err_ = damaris_write(name.c_str(), (void*) dataCol.data.data() ) ;
                    if (damaris_err_ != DAMARIS_OK) {
                        OpmLog::error(fmt::format("ERORR: damariswriter::writeOutput()       : ( rank:{}) damaris_write({}, ...), Damaris Error: {}  ",  rank_, name, damaris_error_string(damaris_err_) ));
                    }
              }
              /*if (name == "PRESSURE") {
                  if (dataCol.data.data() != nullptr) {
...
private:
     ...
     void SetGlobalIndexForDamaris () 
    {
        ...
        damaris_err_ = damaris_write("MPI_RANK", mpiRank.data() ) ;
        if (damaris_err_ != DAMARIS_OK) {
           std::cerr << "ERROR rank =" << rank_ << " : damariswriter::writeOutput() : damaris_write(\"MPI_RANK\""
           << "...), failed. Damaris error = " <<  damaris_error_string(damaris_err_) << std::endl ;
        }
